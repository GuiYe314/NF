// **********************************************************************
// 文件信息
// 文件名(File Name):                BaseComponent.cs
// 作者(Author):                     王云超
// 创建时间(CreateTime):             2024年5月27日 14:35:02
// Unity版本(UnityVersion)：         2021.3.38f1c1
// 脚本描述(Module description):     基础组件
...
namespace NF.Librarie
{
    ...
    [AddComponentMenu("NFramework/Base")]
    public class BaseComponent : NFrameworkComponentBase
    {

        private const int DefaultDpi = 96;  // 默认windows Dpi
        ...
        protected override void Awake()
        {
            ...
            InitVersionHelper();
            InitLogHelper();
            Log.Info("Framework Version: {0}", F.Librarie.Version.FrameworkVersion);
            Log.Info("Version: {0} ({1})", F.Librarie.Version.GameVersion, F.Librarie.Version.InternalGameVersion);
        ...
        private void InitVersionHelper()
        {
            ...
            F.Librarie.Version.IVersionHelper versionHelper = (F.Librarie.Version.IVersionHelper)Activator.CreateInstance(versionHelperType);
            if (versionHelper == null)
            {
                throw new FrameworkException(Utility.Text.Format("Can not create version helper instance '{0}'.", m_VersionHelperTypeName));
            }

            F.Librarie.Version.SetVersionHelper(versionHelper);
        }
        ...
        private void InitLogHelper()
        {
            ...
            FrameworkLog.ILogHelper logHelper = (FrameworkLog.ILogHelper)Activator.CreateInstance(logHelperType);
            if (logHelper == null)
            {
        ...
        private void OnLowMemory()
        {
           ...
           /* ObjectPoolComponent objectPoolComponent = NFrameworkEntry.GetComponent<ObjectPoolComponent>();
